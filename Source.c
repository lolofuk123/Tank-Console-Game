...
double deg_to_rad(double angle);					// konvertuje vlozenu hodnotu uhla v stupnoch na radiany
double km_h_to_m_s(double velocity);				// konvertuje zadanu rychlost v km/h na m/s
...
int generate_random_coordinates();					//generuje random cislo od 80 po 10
void print_starting_position(int player_x, int pc_x);	// vykresli pociatok hry 
...
int main() {
	...
	int hrac_x = generate_random_coordinates();
	int pc_x = generate_random_coordinates();

	print_starting_position(hrac_x, pc_x);
	main_game_function(hrac_x, pc_x);
